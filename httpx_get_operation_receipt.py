...
create_user_response = httpx.post("http://127.0.0.1:8003/api/v1/users", json=create_user_payload)
...
open_credit_card_account_response = httpx.post(
    "http://127.0.0.1:8003/api/v1/accounts/open-credit-card-account",
...
make_purchase_response = httpx.post(
    "http://127.0.0.1:8003/api/v1/operations/make-purchase-operation",
    json=make_purchase_payload,
)
make_purchase_response_data = make_purchase_response.json()

get_receipt_response = httpx.get(
    f"http://127.0.0.1:8003/api/v1/operations/operation-receipt/"
    f"{make_purchase_response_data['operation']['id']}"
)
get_receipt_data = get_receipt_response.json()

print("Get operation receipt response:", get_receipt_data)
print("Get operation receipt status code:", get_receipt_response.status_code)
